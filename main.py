...
from utils import get_difficulty_color, normalize_language, get_platform_file_prefix
...
def add():
    ...
    auto_slug = "_".join([get_platform_file_prefix(platform.lower()), slugify(title)])
    ...
    if existing_problem:
        ...
        if db.problem_has_language(existing_problem, normalized_lang):
            rprint(
                f"[red]Error:[/red] Solution in {language} already exists for this problem."
            )
            raise typer.Exit(1)

        # Create file
        file_path = create_solution_file(
            normalized_platform, slug, normalized_lang, normalized_title
        )
        ...
        rprint(
            f"[green]✓[/green] Added {language} solution to existing problem: {normalized_title}"
        )
...
def list():
    ...
    if not problems:
        rprint(
            "[yellow]No problems found. Use 'add' to create your first problem.[/yellow]"
        )
        return

    # Create table
    table = Table(
        title="CP Tracker - Problems", show_header=True, header_style="bold magenta"
    )
...
def stats():
    ...
    panel = Panel(
        stats_text,
        title="[bold green]CP Tracker Statistics[/bold green]",
        border_style="green",
    )
    console.print(panel)
